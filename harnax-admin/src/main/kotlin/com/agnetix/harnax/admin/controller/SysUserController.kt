...
class SysUserController(
    ...
    ): ResultVo<Page<SysUserResponse>> = onlyAdmin {
        try {
    ...
    ): ResultVo<SysUserResponse> = onlyAdmin {
        try {
            val user = sysUserService.getSysUser(id)
                ?: return@onlyAdmin ResultVo.error(404, messageUtil.getMessage("error.user.notfound"))
    ...
    /**
     * Run [call] when the caller's own row says global admin, and answer
     * `403 / error.user.admin_only` otherwise.
     *
     * The whole domain is gated rather than just the two reads. `updateUser` copies `isAdmin`
     * verbatim, so leaving any write open would keep a one-request path from becoming an admin and
     * back to enumerating the platform; the three `check` probes answer the same question the list
     * does about one named account. The flag is read through [SecurityUtils], i.e. from the row, so
     * neither a token's own `isAdmin` claim nor a principal that resolves to no user at all can
     * decide it.
     */
    private fun <T> onlyAdmin(call: () -> ResultVo<T>): ResultVo<T> = if (SecurityUtils.getCurrentUser()?.isAdmin == 1) {
        call()
    } else {
        log.warn("Rejected a user-domain call by a non-admin caller")
        ResultVo.error(403, messageUtil.getMessage("error.user.admin_only"))
    }
}
