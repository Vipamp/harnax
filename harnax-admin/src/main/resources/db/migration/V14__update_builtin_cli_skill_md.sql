-- Update the built-in harnax-cli skill content to match the latest harnax-cli/SKILL.md.
-- Adds documentation for whoami / skill / skill-repo / cli commands and the new 'skill sync' command.
UPDATE skill s
JOIN skill_repository r ON s.repository_id = r.id
SET s.skillmd = '# Harnax Admin CLI
