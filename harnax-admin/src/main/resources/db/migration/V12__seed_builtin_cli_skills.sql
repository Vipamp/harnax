-- Seed the built-in CLI skills repository and the harnax-cli skill.
-- The repository is identified by the reserved name 'builtin-cli-skills'
-- (see BuiltinRepository.kt) and is read-only via management APIs.
-- Idempotent: both INSERTs skip when the rows already exist.

INSERT INTO skill_repository (tenant_id, name, url, description, status, is_public, creator, active, branch, source_type, source_config, version)
SELECT 1, 'builtin-cli-skills', NULL, '内置 CLI 技能仓库（系统初始化创建，只读）', 1, 1, 'SYSTEM', 1, 'main', 'ZIP', NULL, '1.0.0'
WHERE NOT EXISTS (SELECT 1 FROM skill_repository WHERE name = 'builtin-cli-skills');

INSERT INTO skill (tenant_id, name, repository_id, description, skillmd, resources, version, status, is_public, creator, active)
SELECT 1, 'harnax-cli', r.id, '通过 harnax 命令行管理 Harnax 平台全部资源（Agent、模型、工具、MCP、会话、渠道等）',
'# Harnax Admin CLI
...
```
', '{}', '1.0.0', 1, 1, 'SYSTEM', 1
FROM skill_repository r
WHERE r.name = 'builtin-cli-skills'
  AND NOT EXISTS (SELECT 1 FROM skill s WHERE s.repository_id = r.id AND s.name = 'harnax-cli');
