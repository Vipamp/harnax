...
class SchedulerHousekeepingTest {
    ...
    /**
     * A node that is told not to schedule must not start sweeping either: on a JDBC job store that write
     * surface would otherwise be live on an instance meant to be inert.
     */
    @Test
    fun `a disabled instance registers nothing`() {
        val disabled = SchedulerServiceImpl(
            schedulerFactory,
            agentTaskMapper,
            agentTaskLogMapper,
            routerClient,
