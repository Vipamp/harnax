...
| 类型 | 说明 | 运行时载体 |
|------|------|-----------|
| `BUILTIN` | 内置工具，由 `harnax-tools-buildin` 中的 `ToolBox` Bean 提供，启动时自动同步到数据库 | `ToolBox` 反射注册 |
| `CUSTOM` | 自定义 ToolBox 工具，与 BUILTIN 走同一装配路径（按 `beanName` 查找） | `ToolBox` 反射注册 |
| `HTTP` | HTTP 代理工具，通过 Admin 配置 URL / Method / Headers / JSON Schema，运行时转发请求 | `HttpProxyToolBox` |
| MCP | 外部 MCP 服务（独立于本工具体系，走 `McpConfigAdaptor` 装配） | MCP Client |
...
| `needConfirm` | `false` | **执行前是否需要用户确认**。为 `true` 时运行时生成 ASK 权限规则，每次调用都会暂停并等待用户确认；不检查入参内容，与调用参数无关；在 `BYPASS` 权限模式下会被跳过；也可不改代码，通过 Admin UI 修改 `agent_tool.needConfirm` 或绑定层 `agent_tool_binding.needConfirm` 覆盖，详见 6.4 节 |
...
| `isRequired` | `false` | **是否为必须工具**：`true` 时自动注入所有 Agent（无需在配置中勾选）、不展示在工具管理页面和智能体配置向导中、不可取消；适合所有 Agent 必备的基础工具 |
...
- `ToolSpec`：Agent 配置中的工具引用（`toolId`、`skipIfMissing`、`needConfirm` 覆盖）。
...
| 工具名 | 说明 | readOnly | needConfirm |
|--------|------|----------|-------------|
| `getDate` | 获取当前日期（`yyyy-MM-dd`） | 是 | 否 |
| `getDatetime` | 获取当前时间（`yyyy-MM-dd HH:mm:ss`） | 是 | 否 |

### 4.2 EmailToolBox（`email-tool-box`）

| 工具名 | 说明 | needConfirm |
|--------|------|-------------|
| `sendEmail` | 通过 SMTP 发送邮件，支持纯文本 / HTML 正文 | 是 |
...
实现：`harnax-admin/src/main/kotlin/com/agnetix/harnax/admin/registrar/BuiltinToolAutoRegistrar.kt`

Admin 服务启动（`ApplicationReadyEvent`）时执行 `syncBuiltinTools()`：

1. **Upsert**：每个 `@Tool` 方法对应一条 `agent_tool` 记录（`type=BUILTIN`、`beanName`、`methodName`、`readOnly`、`needConfirm`、`isRequired`、`requiredEnvParamKeys`、`timeoutSeconds` 等），新工具插入 `status=1, active=1`，已有工具仅更新元数据字段，**`status` 永不覆盖**（保留管理员手动禁用状态）。
2. **环境参数同步**：将 `@ToolMeta.envParamDefs` 同步到 `agent_tool_env_param` 表，采用「就地更新 + 新增插入 + 过期删除」策略，保留记录 ID。
3. 已删除的 ToolBox 类不会自动清理数据库记录，需在 UI 手动删除。
...
HarnessAgentLauncher.createAgentBase()
        ...
        │    ├─ status=0（禁用）→ 跳过，并从 Toolkit 移除同名工具
        ...
        │    │      → 按禁用清单 removeTool 逐个剔除
        │    ├─ HTTP → new HttpProxyToolBox(...)（解密 headers）
        │    │      → agentBuilder.registerAgentTool(...)
        │    └─ needConfirm（DB 记录或 toolSpec 覆盖）→ 收集进 needConfirmedTools
        │
        ├─ 回退路径：无 toolSpecs 配置时，注册 ToolRegistry 中全部 ToolBox
        │
...
- **按 beanName 去重**：一个 ToolBox 内的多个 `@Tool` 方法可能对应多条 `agent_tool` 记录，`addTool` 只执行一次，禁用方法通过 `removeTool` 精确剔除。
- **配置来源**：`ToolConfigAdaptorImpl` 优先读取 admin 预解析好的 `toolDetails`（随 AgentSpec 下发），未命中时回退直查数据库。
- **环境参数注入**：`AgentSpecResolver` 将 tool / MCP 绑定的环境变量合并为扁平 Map，封装成 `ToolEnvContext` 挂到 `agentSpec.contextForTools`，构建时注册进 `ToolExecutionContext`，工具方法自动获得注入。
...
- 不改代码也能调整：`agent_tool.needConfirm` 可在 Admin UI 修改，绑定时还可通过 `agent_tool_binding.needConfirm` 按智能体覆盖。
...
- `agentId` / `toolId`：绑定关系；
- `enableSkip`：工具缺失时是否跳过；
- `needConfirm`：绑定级确认覆盖（优先于工具默认值叠加生效）；
...
| `/{id}` | GET | 工具详情 |
| `/update/{id}` | PUT | 更新工具 |
| `/toggle/{id}` | PUT | 启用 / 禁用（status 0/1） |
| `/{id}` | DELETE | 逻辑删除 |
| `/available` | GET | 可用工具列表（智能体配置用，可按 type 过滤） |
| `/builtin` | GET | 启用的内置工具（`is_required = 0` 可选项） |
| `/{id}/required-env-params` | GET | 查询工具必填环境参数 key |

> 备注：`AgentToolService.createAgentTool` 已实现（支持创建 HTTP 等自定义工具，含密钥字段加密），但当前 Controller 尚未暴露 POST 创建接口。

前端管理页面：`harnax-webui/src/pages/tool/`。
...
| `isRequired = true` | 所有 Agent 必备的基础工具，自动注入且不出现在 UI |
...
2. **重启 harnax-admin**：`BuiltinToolAutoRegistrar` 在启动时扫描 `ToolRegistry`，自动将每个 `@Tool` 方法 upsert 为一条 `agent_tool` 记录（`type=BUILTIN`），并把环境参数定义同步到 `agent_tool_env_param`；
3. 验证注册结果：Admin UI「工具管理」页面能看到新工具，或查询 `agent_tool` 表确认记录；
4. **重启 harnax-agent-service**：工具实际执行在 agent-service，未重启时 `ToolRegistry` 中没有新 ToolBox，运行时会因找不到工具而静默跳过（`skipIfMissing`）。

> 同步策略提醒：已有记录的 `status`（启用/禁用）不会被覆盖；代码中删除的 ToolBox 对应记录不会自动清理，需在 UI 手动删除。

### 步骤 6：为智能体绑定工具并配置环境变量

1. Admin UI 进入智能体配置（创建或编辑），在工具选择步骤勾选新工具（`isRequired=true` 的工具无需勾选，自动注入）；
2. 按表单为必填环境参数赋值（引用全局变量或自定义值）；
3. 按需设置 `needConfirm`（执行前二次确认）、`enableSkip`（工具缺失时是否跳过）；
...
| 工具列表页面看不到新工具 | admin 未重启（未同步），或工具 `status=0` / `isRequired=1`（必须工具本就不展示） |
| 运行时静默跳过工具 | agent-service 未重启，`ToolRegistry` 中没有该 ToolBox |
