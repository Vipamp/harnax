...
- **stdio 整体默认关闭**（第十八轮）：一条 stdio 配置不是「一条连接」而是「让 agent-service 起一个进程」，而那个容器挂着宿主 Docker socket，能存下这行就等于能在那台机器上执行命令。`HARNAX_MCP_STDIO_ENABLED`（默认 false）同时管 admin 的 `harnax.mcp.stdio-enabled` 与运行侧的 `harness.mcp-stdio-enabled`：关闭时新建 stdio 与从网络型切进 stdio 都被拒，**存量行仍可编辑、停用、删除，只是永不下发**——把它们锁死会变成「只有能连数据库的人才收拾得了」，比原问题更糟。前端新建表单不再给这一项，编辑表单只在该行本来就是 stdio 时保留它。
