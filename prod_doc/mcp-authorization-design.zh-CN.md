...
5. 分期：P0 安全止血 → P1 身份贯通 → P2 OAuth 客户端 → P3 运行侧注入 → P4 其余方式。P0 与 P1 已完成，P2 四条全部完成（P2-1 / P2-2 / P2-3 / P2-5 之前几轮，P2-4 换发第十八轮），P3 的运行侧注入同轮落地（状态见第 12 节）。剩下的边界是「上游拒了令牌这件事没人知道」——admin 听不到 MCP 服务的 401，所以一条过期又刷不动的凭据只能等用户自己去详情页重授，这一条属 P4 的前置。
...
- **stdio 整体默认关闭**（第十八轮）：一条 stdio 配置不是「一条连接」而是「让 agent-service 起一个进程」，而那个容器挂着宿主 Docker socket，能存下这行就等于能在那台机器上执行命令。`HARNAX_MCP_STDIO_ENABLED`（默认 false）同时管 admin 的 `harnax.mcp.stdio-enabled` 与运行侧的 `harness.mcp-stdio-enabled`：关闭时新建 stdio 与从网络型切进 stdio 都被拒，**存量行仍可编辑、停用、删除，只是永不下发**（第十九轮起连**探测**也不给跑：`McpServerServiceImpl.listTools` 是 admin 唯一会真的执行库里那条 `command` 的路径，它第一件事就是过同一道闸门）——把它们锁死会变成「只有能连数据库的人才收拾得了」，比原问题更糟。前端新建表单不再给这一项，编辑表单只在该行本来就是 stdio 时保留它。
...
授权状态在**详情页**可见（第十四轮随 P2-5 落地）：`OAuthPanel` 挂载即读 `status`，三态徽标（`未授权 / 已授权 / 需重新授权`）+「去授权」+「撤销」。列表页那个徽标**还没做**：原先等的是 P2-4——`NEEDS_CONSENT` 在换发侧落地前没有写入者，批量接口只能显示出两种状态；第十八轮换发落地后这一条不再成立，写状态的人已经有了，剩下的纯粹是给列表加一次查询与一枚徽标的活，见 §12 的 P3 行。i18n 键走 `pages.mcp.oauth.*`（`src/locales/{zh-CN,en-US}/pages.ts`）。

### 9.2 内部端（agent-service，共享密钥）

| 方法 | 路径 | 入参 | 出参 |
|------|------|------|------|
| POST | `/api/admin/internal/mcp/access-token` | `sessionId`, `mcpId`（**没有 user 字段**，身份由 admin 按会话反查） | `accessToken`, `tokenType`, `expiresAtEpochSecond`（epoch 秒而非日期时间：两侧各自比自己本地的时钟，时区可能不同）；无可用授权 401、上游不可达 503、跨租户 403，全都走 `ResultVo.code`，HTTP 恒 200 |
...
| P2 OAuth 客户端 | `V25`/`V26`；发现 + client 注册；授权码 + PKCE 全流程；admin 6 个接口 + 前端授权入口；`mcp_call_log` | 5–7（**四条全部完成**：P2-1 数据模型与管理侧校验、P2-2 发现与客户端登记、P2-3 授权码 + PKCE + 凭据存取与撤销、P2-5 用户侧入口与换票接口（第十四轮，同时闭合 F6）、P2-4 内部换发 + 审计（第十八轮），见 §12.1） | 用 Keycloak/AS 测试实例走通：发现 → 授权 → 存 → 撤销；刷新令牌不出 admin。**仍未跑过真实 AS**，且第十四轮把 `redirect_uri` 的形状改成指向前端路由——AS 侧的登记要人手改一次，改之前第一次授权必然被 AS 拒 |
| P3 运行侧注入 | `authType` 下发；`McpHelper` 挂 customizer；进程内缓存 + 提前刷新 + single-flight；超时补齐 | 3–5（**已完成**，第十八轮） | 已验证（单测）：无用户身份的会话拿不到令牌源、同一会话同一服务只问一次、两个会话共用一台服务各拿各的令牌、被拒后 15s 内不再重复追问、过期条目重新要（`AdminMcpAccessTokenSourceFactoryTest` 6）；换发侧 13 条覆盖 401 / 403 / 503 的分别是谁。**仍未验证**：真实 AS 下的「上游令牌 5 分钟过期时连续调用不掉线」——那要一个能跑的授权服务器，本机既没有 Docker 也没有外网 AS；上游 401 的回执通路还没做，所以一条刷不动的凭据只能由用户自己去详情页重授 |
| P4 其余方式 | `BASIC`、`client_credentials`、mTLS（`customizeStreamableHttpClient`）、DCR | 3–8 | 每种至少一条集成用例 |

总计约 15–25 人日，其中 P0 + P1 已完成（约 4–5），P2 四条与 P3 已完成（P2 合计约 5.5–7；P3 约 3–4），剩下 P4 约 3–8。P1 是 P2/P3 的硬前置，现已就位。
回归面：`McpServerServiceImplTest`、`McpOAuthServiceImplTest`、`McpOAuthUserServiceImplTest`、`McpOAuthStateStoreTest`、`McpOAuthControllerTest`、`SecretFieldEncryptorTest`、`RemoteJsonFetcherTest`、`InternalApiController*`、`harnax-auth` 全套、`DefaultAgentRunnerTest`、`AgentProxyControllerTest`、`McpServerMapperTest` / `AgentMapperTest` / `SessionMapperTest`（`harnax-entity` 那批 Mapper 集成测试需要 Docker；本机 Docker 可用时是全绿的，之前记成「环境失败」的其实是 §11 里那条 mapper XML 解析缺陷）。当前实测数（第十八轮实跑）：`harnax-admin` 单测 **1830**、`harnax-agent-service` **141** 全绿；括号内是历次增量而非累加值（第十八轮加 33：`McpSessionOwnerResolverTest` 10、`McpOAuthUserServiceImplTest.AccessTokenTests` 13、`McpServerServiceImplTest.StdioGateTests` 4、`InternalApiControllerTest` 6；运行侧加 8：`AdminMcpAccessTokenSourceFactoryTest` 6、`McpConfigAdaptorImplTest.AuthTypeTests` 2。更早：P2-2 新增 `McpOAuthServiceImplTest` 30、`RemoteJsonFetcherTest` 9、`SecretFieldEncryptorTest` 的 `ResolveSecretTests` 6，第八轮再加 `McpOAuthServiceImplTest` 17、`RemoteJsonFetcherTest` 5、`AuthTypeTests` 2，第十轮再加 7（掩码回写那一批：`SecretFieldEncryptorTest` 2、`CliServiceImplTest` 1、`EnvVariableServiceImplTest` 1、`AgentToolServiceImplTest` 3），第十一轮再加 `McpOAuthControllerTest` 6——守的是两个 OAuth 接口对外的错误语义，不是发现逻辑；第十二轮 P2-3 再加 64（`McpOAuthUserServiceImplTest` 47、`McpOAuthStateStoreTest` 6、`McpOAuthCallbackControllerTest` 4，以及 `McpOAuthControllerTest` 新增 7 条覆盖按人四个接口），第十三轮再加 6（`RemoteJsonFetcherTest` 14→16 两条真出站用例：失败响应的体要能解析、表单 POST 的参数要按 URL 编码带出去；`McpOAuthUserServiceImplTest` 47→51 四条：宽授予清单、撞唯一键、解不开的密文、没了的注册行），第十四轮净加 8（`McpOAuthUserServiceImplTest` 51→58：回调那 22 条改写成换票 29 条；`McpOAuthControllerTest` 13→16；`McpOAuthServiceImplTest` 47→49 默认回调地址两条；`McpOAuthCallbackControllerTest` 随接口删除 −4））、`harnax-entity` 232 全绿（含第八轮 `McpServerMapperTest` 的两条 `updateOAuthConfig`），`V1..V26` 由一条 `harnax-admin` 的 failsafe IT 在全新 MySQL 8 上按序执行成功；跑法与两个坑（`-am`、`TESTCONTAINERS_RYUK_DISABLED=true`）见 `docs/unit-test-cases.md` §17。
...
- MCP 连接失败目前的口径是「warn 后继续装配，status=0 直接跳过」（见 mcp-management §7.2 第三轮；第十九轮起 admin 组装 spec 时已先扣下禁用行，运行侧这一道只防两侧版本不一致）。**授权失败不属于「可跳过」**：无凭据要显式落到 `NEEDS_CONSENT` 并让用户看到，不能静默少装一个工具。
...
- 本轮之后仍然存在的口子，分「有意留」与「堵不住」记清楚，别让人以为已经闭合：
    ...
    - **Spring Security 上只剩一条 `permitAll()` 业务路径**：`/api/admin/internal/**`，实际把关的是 `InternalApiAuthFilter` 的共享密钥——密钥校验通过就等于「已认证」。第十八轮挂上换发接口时按 §8 第 9 条把这一条重新看了一遍，改了两处：其一，`JwtAuthenticationFilter` 里「把共享密钥当 JWT 递到 `/api/admin/**` 就按内部服务放行」那支不再接受 `application.yml` 的占位默认值——那串字符在本仓库公开着，没改过它的部署等于把全部业务接口（含 MCP / agent / 用户）向读得到仓库的人敞开；代价是**沙箱里的 `harnax-cli` 需要换一个真值进 `ADMIN_INTERNAL_API_SECRET`**，否则它的调用一律 401（admin 启动时有一条 WARN 说这件事）。其二，换发接口在查任何凭据之前先比「会话的租户 == 服务行的租户」，不同直接 403：凭据是按服务行的租户归档的，缺这道校验就等于 B 租户的会话能花掉 A 租户的授权。另一条 `/api/admin/mcp/oauth/callback` 已在第十四轮随 F6 一起删掉（其余 `permitAll` 只有登录 / 验证码 / `mp/auth` / swagger 这些本来就是公开的）。它曾经是个**天然可被外部撞的入口**：撞错的代价只有一句提示页加一条 warn 日志，`McpOAuthStateStore` 的 500 条上限是内存被撑的边界。现在这一面墙搬到了带 JWT 的 `POST /oauth/exchange` 上——没有身份连判断都进不去，500 条上限仍在，但它兜的已不再是陌生人可撞的资源；第十五轮又给「自家用户狂点」这一面加了每人 5 条的上限，于是一个登录用户填不满所有人共用的那 500 条。
...
| P2 | **四条全部完成**（P2-1 数据模型与管理侧校验；P2-2 AS 发现 + 客户端登记 + 管理面前端；P2-3 授权码 + PKCE + 凭据存取与撤销；P2-5 用户侧授权入口 + 带 JWT 的换票接口；P2-4 内部换发 + 审计，第十八轮） | 见下表逐条 |
| P3 | **已完成**（第十八轮，与 P2-4 同一轮：换发接口是运行侧的前提，没有它令牌无处可取） | 下发：`harnax-entity/.../dto/McpDetailDto.kt` 新增 `authType`、`InternalApiController.kt` 带出并扣住 stdio 行、`McpConfigAdaptorImpl.kt` 落到运行时实体（收到空值时 warn）。运行侧：`agent-utils/.../mcp/{McpAccessTokenSource.kt,McpHelper.kt}`（`httpRequestCustomizer` 逐请求挂 `Authorization`、剔除同名的静态头、握手超时放宽到 30s、没有令牌源就不建这个客户端）、`harness-core/.../agent/adaptor/McpAccessTokenSourceFactory.kt`（身份在**建实例时**绑定，理由见 §7.3）、`agent-service/.../adaptor/{AdminMcpAccessTokenSourceFactory.kt}`（按 (会话, 服务) 缓存 + 条带锁 single-flight + 被拒冷却 15s）、`client/AdminApiClient.kt.getMcpAccessToken`；`HarnessAgentLauncher.kt` 装配循环、`HarnessConfig.mcpStdioEnabled` 与 `HarnessAutoConfiguration` 的可选注入。测试：`AdminMcpAccessTokenSourceFactoryTest`（6）、`McpOAuthUserServiceImplTest.AccessTokenTests`（13）、`McpSessionOwnerResolverTest`（10）、`InternalApiControllerTest` 新增 6（`McpAuthDeliveryTests` 3 + `McpAccessTokenTests` 3）。**没做**：真实 AS 的端到端（本机无 Docker 也无外网授权服务器）、上游 401 的回执通路 |
| P4 | 未开始 | — |
...
| P2-4 内部换发 + 审计 | **已完成**（第十八轮，与 P3 同一轮） | 接口 `controller/InternalApiController.kt` 的 `POST /api/admin/internal/mcp/access-token`（体只有 `{sessionId, mcpId}`，**没有 user 字段**：身份由 admin 反查，agent-service 无法指名要谁的凭据）；身份 `util/McpSessionOwnerResolver.kt`（`web-` / `mp-` 走 session 行，`task-` 走 agent_task 行即任务创建人，`chn-` **有意没有身份**；`creator` 在小程序侧存的是用户 id、web 侧是用户名，两种都要试；解出的用户必须落在会话自己的租户里——`sys_user.username` 无唯一索引且 `selectByUsername` 是 `LIMIT 1`）；换发与刷新 `service/impl/McpOAuthUserServiceImpl.kt.accessToken` / `usableToken` / `refreshGrant`（提前 60s 续期、64 条带锁 + 锁内重读防轮换被烧、`invalid_grant` 与「4xx 无 error」判为待授权并写 401、其余失败一律 503 且**不动状态**、会话与服务不同租户直接 403 且不查凭据、撤销过的行只拒绝不改写状态、回答缺 `expires_in` 时沿用库里尚未到期的那个值、解不开的 `client_secret` 按「配置坏了」回答而不是漏一个未兜住的 500）；审计 `mcp_call_log` 每次决策一行（`ISSUE` / `REFRESH` × `OK` / `NEEDS_CONSENT` / `ERROR` + 延迟，**不含令牌与 scope**，写失败不影响调用）；DTO `harnax-entity/.../dto/McpAccessTokenResponse.kt`（跨服务契约，所以放 entity 模块；过期用 epoch 秒，两侧时区可能不同）。测试：`McpOAuthUserServiceImplTest.AccessTokenTests`（13）、`McpSessionOwnerResolverTest`（10） |
| P2-5 用户侧授权入口 + 换票接口 | **已完成**（第十四轮，与闭合 F6 同一轮：换票接口是这一项的前提，没有它「点一下授权」无处可去） | 落地页 `harnax-webui/src/pages/mcp/oauth-callback.tsx`（`useEffect` 第一句读 query 再 `history.replace` 抹掉，`code` 与 `state` 只停在 effect 里的一个局部对象上、立刻发出去，不进 `useState` 也不落 `localStorage`；成功回显授予的 scope 与过期时间，失败原样显示后端那句话——重新发起要去点详情页那个按钮，这里重发同一个 code 不会有别的下场，所以页面上只有「返回 MCP」一个动作）；路由 `harnax-webui/config/routes.ts`（`/mcp/oauth/callback`、`layout: false`、排在 `path: '*'` 之前）；`harnax-webui/src/app.tsx` 的 `loginRedirect`——未登录时跳去 `/login?redirect=` 会把 query 丢掉，否则 `code` 会出现在登录页地址栏与登录后的回跳参数里；按人授权块 `pages/mcp/components/OAuthPanel.tsx`（挂载即读 `status`，三态徽标 + 「去授权」同页跳转 + `Modal.confirm` 包住的「撤销」，并回显 scopes / 过期时间 / `last_error`；`callbackUrl` 与登记值逐字符比对的提醒也在这；第十五轮起**读不到 `status` 就不画徽标**——`grantUnknown` 为真时三态一个都不选，「还没读到」不能替用户断言「未授权」，撤销成功后重读失败更是如此）；服务调用 `services/ant-design-pro/mcp.ts` 四个（`getMcpOAuthAuthorizeUrl` / `exchangeMcpOAuthCode` / `getMcpOAuthStatus` / `revokeMcpOAuth`）+ `src/typings.d.ts` 对应类型；文案 `locales/{zh-CN,en-US}/pages.ts` 的 `pages.mcp.oauth.*` 由 52 键扩到 76 键（实测两份各 76，同数同序；第十八轮把 `runtimePending` 改成 `runtimeNote` 并新增 `noteTitle`）。**没做的**：列表页徽标（随 P3，理由见 §9.1）、`scope` 覆盖参数（后端支持，UI 不给入口） |
...
| 配置下发 | `harnax-admin/src/main/kotlin/com/agnetix/harnax/admin/controller/InternalApiController.kt`（`agent-spec`、`plainConfigJson`、`api-keys/validate`；第十八轮起还有 `mcp/access-token` 与 stdio 扣留，扣留规则的真源在 `admin/service/McpStdioPolicy.kt`）、`harnax-entity/src/main/kotlin/com/agnetix/harnax/entity/dto/McpDetailDto.kt`（现带 `authType`）、同目录 `McpAccessTokenResponse.kt`、`harnax-admin/.../util/McpSessionOwnerResolver.kt` |
| 运行侧装配 | `harnax-agent/harnax-harness-core/src/main/kotlin/com/agnetix/harnax/harness/HarnessAgentLauncher.kt`、`harnax-agent/harnax-agent-utils/src/main/kotlin/com/agnetix/harnax/agent/adaptor/mcp/McpHelper.kt`（含它接受的 `McpAccessTokenSource`）、`harnax-agent/harnax-harness-core/src/main/kotlin/com/agnetix/harnax/agent/adaptor/McpAccessTokenSourceFactory.kt` 与它的实现 `harnax-agent/harnax-agent-service/.../adaptor/AdminMcpAccessTokenSourceFactory.kt`、`harnax-agent/harnax-harness-core/src/main/kotlin/com/agnetix/harnax/harness/mcp/PlaintextMcpConfigDecryptor.kt` |
