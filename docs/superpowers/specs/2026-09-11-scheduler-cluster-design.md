...
- **发布 1 的实际位置（修正 C，历史）**：那时数据源仍是 `harnax_admin`，所以 11 张 `QRTZ_*` 与本服务自己的历史表都落在 admin 库里。Quartz 的行是可再生数据（reconcile 按 `agent_task` 重建全部 job），所以 S3 切库时不搬 `QRTZ_*`；**又因为用户确认无历史数据、D8 的迁移整项取消**，`harnax_admin` 里那三张业务表与 11 张 `QRTZ_*` 在切口后没有任何活着的读者，**直接 DROP，不需要观察期**（原计划的"另存 DROP 脚本、运维签认后执行"随之取消；三张业务表那条 DROP 后来以 admin `V39` 合入仓库，见 §9 F16）。切口步骤正文：`docs/deploy-harnax-scheduler.md` 的「发布 2 切口」。
...
`harnax_admin` 里的三张业务表与 11 张 `QRTZ_*` 在发布 2 的切口之后**不留观察期**：原 M5.1 那套"晚于 scheduler 稳定上线、观察过至少一个完整 cron 周期、运维签认后才合入"的硬约束是**为搬了数据的表设计的**，而这一版什么都不搬（D8 已取消），所以这些表在切口那一刻起就没有任何活着的读者，**切口步骤里直接 DROP**（`docs/deploy-harnax-scheduler.md` 的「发布 2 切口」第 8 步）。当时把这条 SQL 留在仓库外，是因为它唯一的作用是给回滚一个可指的地方；三张业务表后来仍然合进了仓库（admin `V39__drop_agent_task_tables.sql`，见 §9 F16），因为不合进去的代价更实在——每一次全新部署都会重放出这三张空表，而运维在这个库里找任务表会找到一张空的。合进来的另一面是那条回滚从此要先重建表才有地方可指。`QRTZ_*` 保持原样，由运维就地删。
...
16. **F16 全新部署会把三张孤儿任务表重放进 `harnax_admin`（✅ 2026-09-22 修：`harnax-admin/src/main/resources/db/migration/V39__drop_agent_task_tables.sql`）**。事实：admin 的 Flyway 序列是历史序列，V1 建过 `agent_task` / `agent_task_log` / `agent_task_execution`，所以一个**空库**从头重放之后照样有这三张表（2026-09-16 实测 `harnax_admin` 37 张表，其中这三张行数为 0）。发布 2 之后没有任何代码读它们（`harnax-admin/src/main` 对三个 mapper 名零命中，见 §11 第 6 条），真身在 `harnax_scheduler`。§10 写的"旧库的三张业务表切口后直接 DROP"当时交给运维就地执行，于是全新部署既没有可迁的数据、也没有任何一步去 DROP，这三张空表就留下一个读起来像"还没搬干净"的残影。**危害不是功能**（0 行、无人读、不产生错误），是**权威库的可辨识性**：运维照 `harnax_admin` 找任务表会找到一张空的。**修法照原登记落地**：一条 `DROP TABLE IF EXISTS` ×3 的迁移，不动任何历史迁移文件，让重放出来的新库与手工清过的老库收敛到同一终态（原登记写的号位是 V29，那个号已被占用，落地为 V39）。**唯一代价要说清**：`docs/deploy-harnax-scheduler.md`「发布 2 切口」回滚三件套里"数据源指回 `harnax_admin`"这一步从此要求先把三张表建回来——那条退路的截止日期原本握在运维手里，现在由 V39 自动到期。
17. **F17 两副本冷启动时系统 sweep 撞重复键，一条带 SQL 细节的 WARN（🟡 2026-09-16 实测，已自愈，终态正确）**。事实：`SchedulerServiceImpl.registerSweep` 是 `if (scheduler.checkExists(jobKey)) 沿用 else scheduleJob(...)`（`:250-262`），两副本同一瞬间起来时这个 check-then-act 没有互斥——`13:39:11.690` B 的 housekeeping 读到了 A 已写入的行，走静默分支 `The housekeeping sweep is already registered (every 300s), leaving it alone`；`13:39:11.764` B 的对账那条读在 A 写入之前，于是撞主键：`The task reconcile sweep could not be registered: Couldn't store job: Duplicate entry 'HarnaxScheduler-AgentTaskScheduleReconcile-SchedulerSystemGroup' for key 'qrtz_job_details.PRIMARY'`。**恢复路径按设计兑现**：这条把 `systemSweepsPending` 留着，`13:39:11.784` Quartz 的 initial-load 线程重试时 `checkExists` 已经为真，日志改为 `already registered (every 60s), leaving it alone`。**终态实测正确**：`QRTZ_JOB_DETAILS` 的 `SchedulerSystemGroup` 恰好 2 个 job、`QRTZ_TRIGGERS` 恰好 2 个 WAITING（不是 4），存活期内 8 分钟 `scheduler.reconcile.rounds{outcome=success}`=8 且没有 failure/drift 采样。**危害只有一条**：一次冷启动会把 `Duplicate entry ... qrtz_job_details.PRIMARY` 这种 SQL 细节印在运维读得到的 WARN 里，看上去像故障。修法同 housekeeping 的形状——注册前先 `checkExists`，或把这条路径上的重复键异常并进"already registered"分支。**登记未修**：它不影响任何用户可见行为，且要真做到不吵得改 `registerSweep` 的异常分类，属于另开一刀的量级。

## 10. 数据迁移（D8）

> **状态：整节取消。** 用户确认这次切换**没有历史数据**，于是 D8 剩下的部分（把 `agent_task` 的定义搬过去）也一起去掉：发布 2 的切口**不迁任何东西**。

- 因此这一节原来写的东西全部作废，且**不再有对应物**：没有 `INSERT ... SELECT` 脚本、没有"目标表已有行就跳过"的幂等守卫、没有末尾那条自校验 SELECT，也没有"历史日志变空 / 最近运行两列变空"这类要写进公告的损失——没有东西可失去。仓库里不留迁移 SQL；旧表那条 DROP 当时也不留（后来三张业务表的那一刀以 admin `V39` 合入，见 §9 F16，`QRTZ_*` 仍在运维手里）。
...
- 旧库 `harnax_admin` 的三张业务表与 11 张 `QRTZ_*`：**切口后直接 DROP，不留观察期**（原来那条"运维签认 + 观察一个完整 cron 周期"是为搬了数据的表设的）。三张业务表由 admin 的 `V39__drop_agent_task_tables.sql` 删，`QRTZ_*` 由运维就地删——后者只在跑过发布 1 的安装里存在过。
- 切口顺序正文：`docs/deploy-harnax-scheduler.md` 的「发布 2 切口」。

## 11. 验收标准

1. `mvn -o -pl harnax-scheduler verify -Pintegration-test` 通过，IT-1/IT-2/IT-5 全绿。**发布 1 只交付到"这三个类写好了"**：交付机器没有 Docker 守护进程，它们一次都没执行过，所以这一条**现在是未决项，不是已完成项**——它要有 Docker 的机器（本机或验收机）跑一次才算数。**发布 2 让这条的未决集合变大而不是变小**：新增的 IT-3（`AgentTaskOwnerScopeIT`，属主/可见性规则在真库上的越权断言）与 `AgentTaskMapperSemanticsIT`（三条 mapper 语义测试随域迁来）同样是"能编译、从未执行"。IT-4 的依赖（S4 的 one-shot 合并）已由发布 3 满足、但用例仍未编写，IT-6/IT-7 属中断命中与超时竞争两批，都不在本条之内。**2026-09-16 本机已补跑（Docker 29.1.3 / compose v2.40.3）：`mvn -pl harnax-scheduler -Pintegration-test test-compile failsafe:integration-test failsafe:verify` → 49 tests、0 failures、0 errors、BUILD SUCCESS**（`AgentTaskMapperSemanticsIT` 40、`AgentTaskOwnerScopeIT` 6、IT-1/IT-2/IT-5 各 1）。这一跑把三处"没有 Docker 就永远不会现形"的装配缺陷照了出来，都已修：①`schema-it.sql` 有意留着的那张 IT 私表让 Flyway 拒绝迁移一个"非空但没有历史表"的库，5 个类 49 条全部以 `Failed to load ApplicationContext` 告终 → IT profile 加 `baseline-on-migrate: true` + `baseline-version: 0`（版本必须是 0，默认值 1 会把 V1 记成已应用、于是 `QRTZ_*` 一张都不建）；②`ClusterSingleFireIT` 先建两个同名节点再一起 `start()`，而 `StdSchedulerFactory.getScheduler()` 是按**名字**从 `SchedulerRepository` 取回那个活着的实例（quartz-2.5.2-sources `StdSchedulerFactory.java:1553-1572`），第二个节点因此根本没被建出来——日志里 `cluster-node-one started.` 打了两遍、`QRTZ_SCHEDULER_STATE` 只有一行 → 改成起完第一个就把它的名字从仓储里 `remove` 再建第二个（集群的定义就是共用 `instanceName`）；③IT 进程在 +08:00 而 Testcontainers 的 MySQL 是 UTC，mapper 用 `NOW()` 比较 Java 写进去的 `DATETIME`，所有"很老"的种子行都被记成 8 小时后的未来，`expireStale` 回收 0 行 → failsafe 加 `-Duser.timezone=UTC`（真部署两端同为 Asia/Shanghai，见 compose 的 `TZ` 与 JDBC 的 `serverTimezone`）。
2. `docker-compose -f docker-new/docker-compose.yml up -d --scale scheduler=2` 后 `QRTZ_SCHEDULER_STATE` 有两个实例的 `INSTANCE_NAME`，且每行的 `LAST_CHECKIN_TIME` 每 15s 各自前进（发布 2 起这张表在 **`harnax_scheduler`** 库，`roll-scheduler.sh` 的回读已跟着改；**行数不等于副本数**，见上）；kill 其一，另一个接管它未完成的 trigger。接管窗口是**算出来的 22.5~52.5s**，不是范围猜的：死节点那行的 `LAST_CHECKIN_TIME` + `CHECKIN_INTERVAL` 15000ms + Quartz 硬编码的 7500ms = **22.5s** 之后才被 `calcFailedIfAfter` 判为失效，而对端只在自己的 ClusterManager 线程醒来时求值这个式子（睡的就是一个 checkin 周期）→ **+至多 15s**；那一轮对端自己被慢库拖过一整个周期时 `max()` 取 30s 而非 15s → **再 +15s**，最坏 52.5s。整个窗口都落在 `misfireThreshold: 60000` 之内，所以**单节点死亡不丢触发**，而一次全 service 的 `--force-recreate`（最长 400s）必然越过它——那条边由 `roll-scheduler.sh` 关住（见 6.3）。**2026-09-16 全新部署已实测**：`QRTZ_SCHEDULER_STATE` 两行、各自每 15s 前进；`docker kill docker-new-scheduler-2`（SIGKILL，等价节点猝死）后存活副本 **21s**（轮询粒度 5s）把行数收回到 1，并打出 `ClusterManager: detected 1 failed or restarted instances` → `Scanning for instance "…"s failed in-progress jobs` → `……Freed 1 acquired trigger(s)`，随后它的对账轮次计数继续按 60s 前进（未接管成双跑）。自 kill 起算 21s 换算到"自最后一次心跳起算"是 21~36s，与上面那台算术的 22.5~52.5s 下沿吻合——**注意这条窗口本来就是从心跳算起、不是从 kill 算起**。`up -d --scale scheduler=2` 后重新入集群（两行、最大心跳滞后 8.9s）。
