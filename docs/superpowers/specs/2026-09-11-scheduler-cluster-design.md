...
16. **F16 全新部署会把三张孤儿任务表重放进 `harnax_admin`（🟡 2026-09-16 实测，与 §10 的"切口后直接 DROP"不吻合）**。事实：admin 的 Flyway 序列 V1..V28 是历史序列，中间那几条建过 `agent_task` / `agent_task_log` / `agent_task_execution`，所以一个**空库**从头重放之后照样有这三张表（实测 `harnax_admin` 37 张表，其中这三张行数为 0）。发布 2 之后没有任何代码读它们（`harnax-admin/src/main` 对三个 mapper 名零命中，见 §11 第 6 条），真身在 `harnax_scheduler`。§10 写的"旧库的三张业务表与 11 张 `QRTZ_*` 切口后直接 DROP"针对的是**跑过老版本的库**，而且那条 DROP 明确交给运维就地执行、仓库里不留脚本；全新部署既没有可迁的数据、也没有任何一步去 DROP，于是这三张空表就在这里留下一个读起来像"还没搬干净"的残影。**危害不是功能**（0 行、无人读、不产生错误），是**权威库的可辨识性**：运维照 `harnax_admin` 找任务表会找到一张空的。**登记未修**，理由有二：这条部署的验收不依赖它；真正的修法（admin 加一条 `V29__drop_agent_task_domain.sql`，三张 `DROP TABLE IF EXISTS`，让全新库与老库收敛到同一终态）要重新构建 admin 镜像才有意义，不该塞进一次部署的尾巴。谁接手就照这个修法做，别去改历史迁移文件。
