...
class StartupUrlPrinter(
    ...
    fun onReady(event: ApplicationReadyEvent) {
        ...
        log.info("  Monitor UI  : {}/ui?token=<jwt or api key>", baseUrl)
        log.info("  Instances   : {}/api/router/monitor/instances", baseUrl)
        log.info("  Call logs   : {}/api/router/monitor/call-logs", baseUrl)
        log.info("                (both need Authorization: Bearer <jwt> or X-Api-Key)")
        log.info("  Ready       : {}/actuator/health/readiness", baseUrl)
        log.info("  Agents      : {}/api/router/health", baseUrl)
