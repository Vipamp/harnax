 ...
 *
 * TODO: 消息接收功能需要根据飞书 SDK 实际的事件类型进一步完善
 * 当前版本可以建立 WebSocket 连接并发送消息
 */
class FeishuWebSocketMode(
    ...
    override fun stop(channel: ChannelSpec) {
        ...
        if (wsClient != null) {
            try {
                // 飞书 SDK 的 WsClient 没有显式的 stop 方法
                // 连接会在应用关闭时自动断开
    ...
    override suspend fun sendMessage(channel: ChannelSpec, sessionId: String, message: String) {
        ...
        val messageBody = mapOf(
            ...
            "msg_type" to "text",
            "content" to contentJson,  // ← 必须是 JSON 字符串
        )
