package com.agnetix.harnax.channel.dingtalk.client
 ...
 * Unified HTTP client based on Spring WebFlux WebClient, used to POST replies to
 * DingTalk bot session webhooks.
 *
 * Features:
 * - Timeout configuration (connect 5s, read 10s)
 * - Automatic retry (up to 3 times, exponential backoff) on 429/5xx
 */
        ...
        return executeWithRetry(url) { attempt ->
            ...
            try {
                ...
                if (statusCode in 200..299) {
                    PlatformResponse.Success(statusCode = statusCode, body = responseBody)
                } else {
                    PlatformResponse.Error(statusCode = statusCode, body = responseBody)
                }
            } catch (e: Exception) {
    ...
    private suspend fun executeWithRetry(
        ...
        for (attempt in 1..maxRetries) {
            try {
                ...
                if (response is PlatformResponse.Success) return response

                lastError = response as PlatformResponse.Error
                if (!isRetryableError(response.statusCode)) return response
                ...
                lastError = PlatformResponse.Error(statusCode = 0, body = "", exception = e)
                if (attempt < maxRetries) {
                    val delayMs = calculateDelay(attempt)
