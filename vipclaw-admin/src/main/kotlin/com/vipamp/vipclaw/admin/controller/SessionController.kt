...
class SessionController(
    ...
    fun getSessionPage(
        ...
        return try {
            val page = sessionService.getSessionPage(keyword, status, pageNum ?: 1, pageSize ?: 10)
            val responsePage = Page<SessionResponse>(page.current, page.size)
            responsePage.total = page.total
            responsePage.size = page.size
            responsePage.current = page.current
            responsePage.pages = page.pages
            responsePage.records = page.records.map { sessionService.convertToResponse(it) }
    ...
    fun toggleSession(
        @Parameter(description = "会话 ID") @PathVariable(name = "sessionId") sessionId: Long,
        @Parameter(description = "会话状态") @RequestParam(name = "status") status: Int
    ): ResultVo<Void> {
        return try {
            if (sessionService.toggleSessionStatus(sessionId, status)) ResultVo.success() else ResultVo.error("更新会话失败")
        } catch (e: Exception) {
            log.error("更新会话失败", e)
    ...
    fun deleteSession(
        ...
        return try {
            if (sessionService.deleteSession(sessionId)) ResultVo.success() else ResultVo.error("删除会话失败")
        } catch (e: Exception) {
            log.error("删除会话失败", e)
