...
class ModelProviderServiceImplTest {
    ...
    inner class CreateTests {
        ...
        fun `create should create provider successfully`() {
            ...
            assertEquals("sk****2345", result.apiKey) // 脱敏后的 API Key
    ...
    inner class UpdateTests {
        ...
        @Test
        @DisplayName("update - 脱敏格式的 apiKey 不更新")
        fun `update should not update apiKey when masked format`() {
            // Given
            val id = 1L
            val existingProvider = ModelProvider().apply {
                this.id = id
                this.name = "dashscope"
                this.displayName = "阿里云百炼"
                this.apiKey = "sk-original-key-12345"
                this.active = 1
            }
            ...
            // Then
            assertEquals("sk****2345", result.apiKey) // 应该保持旧的 apiKey（脱敏后）
            verify(modelProviderMapper).updateById(any())
        }
    }
    ...
    inner class PageTests {
        ...
        fun `page should return paginated results`() {
            ...
            whenever(modelProviderMapper.selectModelProviderList(null, null, "admin")).thenReturn(providers)
            whenever(jwtUtil.getUsernameFromToken(any())).thenReturn("admin")

            // When
            val result = service.page(page, null, null)
        ...
        fun `page should filter by name`() {
            ...
            whenever(modelProviderMapper.selectModelProviderList("dash", null, "admin")).thenReturn(providers)
            whenever(jwtUtil.getUsernameFromToken(any())).thenReturn("admin")

            // When
            val result = service.page(page, "dash", null)

            // Then
            assertEquals(1, result.total)
            assertEquals("dashscope", result.records[0].name)

            verify(modelProviderMapper).selectModelProviderList("dash", null, "admin")
        }
    }
    ...
    inner class GetDetailTests {
        ...
        fun `getDetail should return provider detail`() {
            ...
            assertNotNull(result)
            assertEquals("dashscope", result.name)
            assertEquals("sk****2345", result.apiKey) // 脱敏
        }
