...
| `password`        | VARCHAR  | 100 | ❌ 否 | -                           | 密码（前端先base64，再BCrypt 加密存储） | NOT NULL             |
| `nickname`        | VARCHAR  | 50  | ❌ 否 | -                           | 昵称（显示名称）                   | NOT NULL             |
| `email`           | VARCHAR  | 100 | ❌ 否 | -                           | 邮箱地址                       | NOT NULL             |
| `phone`           | VARCHAR  | 20  | ❌ 否 | -                           | 手机号                        | NOT NULL             |
...
| `last_login_time` | DATETIME | -   | ✅ 是 | NULL                        | 最近一次登陆时间                   | -                    |
...
- `email` 和 `phone` 字段虽然在业务层面应该唯一,但数据库层面暂未添加唯一索引
- 如需添加唯一约束,可执行: `ALTER TABLE sys_user ADD UNIQUE KEY uk_email (email);`
- 如需添加唯一约束,可执行: `ALTER TABLE sys_user ADD UNIQUE KEY uk_phone (phone);`
...
| `password` | 用户登录密码，BCrypt 加密存储 | - 长度：6-100 字符<br>- HTTPS 传输明文，后端 BCrypt 加密                  | `123456` → `$2a$10$...`         |
...
| `avatar` | 用户头像 URL | - URL 格式校验<br>- 最大长度：255 字符<br>- 默认值：NULL | `https://example.com/avatar.jpg` |
...
| `last_login_time` | 登录时自动更新，初始值为 NULL               | 记录用户最近一次登录时间，新用户创建时为 NULL |
...
| `password` | String? | ❌ 否 | - 长度：6-100              | 密码（可选，提供则加密更新）  |
| `is_admin` | Int?    | ❌ 否 | - 枚举：0,1                | 是否管理员（可选）       |
...
2. 查询用户
   ...
   ↓
3. 关联检查（可选）
   - 检查用户是否有创建的 Agent、Model 等关联资源
   - 如有重要关联资源，提示管理员确认或进行数据迁移
   - 注意：当前实现不做强制拦截，仅做逻辑删除
   ↓
4. 逻辑删除（非物理删除）
...
(
    ...
    `is_admin`        TINYINT(2) NOT NULL DEFAULT 0 COMMENT '是否是管理员（0:否，1:是）',
    `active`          TINYINT(2) NOT NULL DEFAULT 1 COMMENT '逻辑删除标识 (0:已删除 1:正常)',
    `last_login_time` DATETIME     DEFAULT NULL COMMENT '最近一次登录时间，初始为 NULL',
